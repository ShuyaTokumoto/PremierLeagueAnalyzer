...
def compute_metrics(df: pd.DataFrame, min_minutes: int = 450) -> pd.DataFrame:
    ...
    #
    # 旧版の問題:
    #   - influence（攻撃成分大）を主指標に使っていた
    #   - FWDのポジション補正が最大(1.5)でサラー・イサク等が最高位に
    #
    # 改善方針:
    #   1. influence を完全除外 → 純守備フィールドのみ使用
    #   2. ポジション補正を逆転（GK/DFを高く、FWを低く）
    #   3. 守備の本質: CS率・失点抑制・GKセーブ・守備アクション数
    #
    # 計算式:
    #   GK:  saves/90 × 1.5 + CS率 × 3.0 − GC/90 × 0.8 + def_act/90 × 0.1
    #   DEF: def_act/90 × 0.20 + CS率 × 3.0 − GC/90 × 0.8
    #   MID: def_act/90 × 0.15 + CS率 × 1.5
    #   FWD: def_act/90 × 0.10 + CS率 × 0.5
    #   → さらにポジション補正: GK=1.2, DEF=1.1, MID=0.8, FWD=0.4
    #
    # 結果: saves/CS/def_actionsと強い正相関、
    #       influence/threat/xAと強い負相関（攻撃選手が低くなる）
    # ─────────────────────────────────────────────────
    ...
    # CS率: クリーンシート数 / 出場試合数（38試合換算）
    cs_rate_     = df["clean_sheets"] / (p90 / 38 * 38).clip(lower=1)
    def_actions  = (
        df["tackles"]
        + df["recoveries"]
