...
def fetch_vaastav_players(season: str) -> tuple[pd.DataFrame | None, str]:
    """
...
@st.cache_data(ttl=1800, show_spinner=False)
def fetch_vaastav_gw(season: str) -> pd.DataFrame | None:
...
@st.cache_data(ttl=1800, show_spinner=False)
def fetch_vaastav_teams(season: str) -> pd.DataFrame | None:
...
def compute_metrics(df: pd.DataFrame, min_minutes: int = 450) -> pd.DataFrame:
    ...
    # ── ② 守備プロセス ──────────────────────────────
    pos_def_w = {"GK": 0.8, "DEF": 1.0, "MID": 1.3, "FWD": 1.5}
    df["_pos_w_def"] = df["position"].map(pos_def_w).fillna(1.0)
    ...
    def_p90  = def_actions / p90
    inf_max  = df["influence"].max()
    inf_n    = df["influence"] / (inf_max if inf_max > 0 else 1)
